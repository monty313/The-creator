# Tweak record: `note__local_desktop_factory_full_GV-014-XAU-L1_md`

**Title:** GV-014-XAU-L1.md  
**Kind:** note  
**Source language:** `C:\Users\user\OneDrive\Desktop\The Creator\strategies\local_desktop\factory_full\GV-014-XAU-L1.md`  
**Adapter profile:** `gv014`  
...
Profile `gv014` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 72.9632 |
| Total trades (sum set×mode) | 415 |
| Total return % (avg set×mode) | -0.1866 |
| Max drawdown % (avg) | 0.4953 |
| Profit factor (avg) | 0.4464 |
| Sharpe (avg) | -0.9399 |
| Aggregate score | 44.3254 |
| Runs (set×mode) | 8 |

**Delta win rate:** +51.8928 pp vs baseline.
...
**MC rank (by bootstrap median terminal):** **33** / 139  
...
| Historical terminal (compound order of book) | 0.985060× |
| Historical max DD | 1.9402% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.985491× |
| Mean terminal wealth | 0.985325× |
| p05 terminal | 0.962538× |
| p25 terminal | 0.976574× |
| p75 terminal | 0.994507× |
| p95 terminal | 1.006019× |
| P(loss) = P(terminal < 1) | 86.60% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 2.2132% |
| p95 path max DD | 4.1470% |
