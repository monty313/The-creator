# Tweak record: `note__the_truth_main_extra_strategy_S2_bb_trend_reversion_md`

**Title:** strategy_S2_bb_trend_reversion.md  
**Kind:** note  
**Source language:** `C:\Users\user\OneDrive\Desktop\The Creator\strategies\the_truth_main_extra\strategy_S2_bb_trend_reversion.md`  
**Adapter profile:** `truth_s2_bb`  
...
Profile `truth_s2_bb` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 72.9201 |
| Total trades (sum set×mode) | 481 |
| Total return % (avg set×mode) | -0.2396 |
| Max drawdown % (avg) | 0.5789 |
| Profit factor (avg) | 0.7047 |
| Sharpe (avg) | -1.0059 |
| Aggregate score | 70.0888 |
| Runs (set×mode) | 8 |

**Delta win rate:** +37.3917 pp vs baseline.
...
**MC rank (by bootstrap median terminal):** **37** / 139  
...
| Historical terminal (compound order of book) | 0.980818× |
| Historical max DD | 2.7937% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.980315× |
| Mean terminal wealth | 0.980578× |
| p05 terminal | 0.956767× |
| p25 terminal | 0.970389× |
| p75 terminal | 0.990631× |
| p95 terminal | 1.004477× |
| P(loss) = P(terminal < 1) | 90.60% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 2.7047% |
| p95 path max DD | 4.7346% |
