# Tweak record: `mt__SMA_Fan_MTF_BBExit_v1`

**Title:** SMA_Fan_MTF_BBExit_v1  
**Kind:** mt  
**Source language:** `strategies/language/01_METATRADER_INDEX.md :: SMA_Fan_MTF_BBExit_v1 (.mq5)`  
**Adapter profile:** `sma_scalp`  
...
Profile `sma_scalp` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 78.3520 |
| Total trades (sum set×mode) | 250 |
| Total return % (avg set×mode) | 0.1241 |
| Max drawdown % (avg) | 0.3991 |
| Profit factor (avg) | 13.3673 |
| Sharpe (avg) | 1.2273 |
| Aggregate score | 1336.7516 |
| Runs (set×mode) | 8 |

**Delta win rate:** +36.1417 pp vs baseline.
...
**Family id:** `mt__SMA_Fan_MTF_BBExit_v1`  
**MC rank (by bootstrap median terminal):** **1** / 139  
...
| Historical terminal (compound order of book) | 1.009880× |
| Historical max DD | 0.9388% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 1.010139× |
| Mean terminal wealth | 1.010046× |
| p05 terminal | 0.991228× |
| p25 terminal | 1.003040× |
| p75 terminal | 1.017238× |
| p95 terminal | 1.027604× |
| P(loss) = P(terminal < 1) | 19.00% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 0.8596% |
| p95 path max DD | 1.6800% |
