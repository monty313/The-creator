# Tweak record: `mt__KineticEdgeEA`

**Title:** KineticEdgeEA  
**Kind:** mt  
**Source language:** `strategies/language/01_METATRADER_INDEX.md :: KineticEdgeEA (.mq5)`  
**Adapter profile:** `kinetic`  
...
Profile `kinetic` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 68.6435 |
| Total trades (sum set×mode) | 522 |
| Total return % (avg set×mode) | -0.6665 |
| Max drawdown % (avg) | 0.8389 |
| Profit factor (avg) | 0.5838 |
| Sharpe (avg) | -3.3283 |
| Aggregate score | 57.5076 |
| Runs (set×mode) | 8 |

**Delta win rate:** +26.5027 pp vs baseline.
...
**MC rank (by bootstrap median terminal):** **82** / 139  
...
| Historical terminal (compound order of book) | 0.947607× |
| Historical max DD | 5.7549% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.947230× |
| Mean terminal wealth | 0.947673× |
| p05 terminal | 0.926013× |
| p25 terminal | 0.938389× |
| p75 terminal | 0.956373× |
| p95 terminal | 0.970210× |
| P(loss) = P(terminal < 1) | 99.90% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 5.5314% |
| p95 path max DD | 7.5769% |
