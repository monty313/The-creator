# Tweak record: `mt__CCI_ShiftedSMA_Signal_3D`

**Title:** CCI_ShiftedSMA_Signal_3D  
**Kind:** mt  
**Source language:** `strategies/language/01_METATRADER_INDEX.md :: CCI_ShiftedSMA_Signal_3D (.mq5)`  
**Adapter profile:** `ati_sma`  
...
Profile `ati_sma` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 71.8321 |
| Total trades (sum set×mode) | 857 |
| Total return % (avg set×mode) | -0.5250 |
| Max drawdown % (avg) | 1.0694 |
| Profit factor (avg) | 0.8714 |
| Sharpe (avg) | -2.4959 |
| Aggregate score | 86.3477 |
| Runs (set×mode) | 8 |

**Delta win rate:** +29.2707 pp vs baseline.
...
**MC rank (by bootstrap median terminal):** **76** / 139  
...
| Historical terminal (compound order of book) | 0.958407× |
| Historical max DD | 4.5036% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.957473× |
| Mean terminal wealth | 0.957783× |
| p05 terminal | 0.927847× |
| p25 terminal | 0.946770× |
| p75 terminal | 0.969543× |
| p95 terminal | 0.986828× |
| P(loss) = P(terminal < 1) | 98.90% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 4.8536% |
| p95 path max DD | 7.5497% |
