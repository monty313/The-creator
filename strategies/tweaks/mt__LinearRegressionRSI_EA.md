# Tweak record: `mt__LinearRegressionRSI_EA`

**Title:** LinearRegressionRSI_EA  
**Kind:** mt  
**Source language:** `strategies/language/01_METATRADER_INDEX.md :: LinearRegressionRSI_EA (.mq5)`  
**Adapter profile:** `linreg`  
...
Profile `linreg` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 70.7195 |
| Total trades (sum set×mode) | 251 |
| Total return % (avg set×mode) | -0.2416 |
| Max drawdown % (avg) | 0.4290 |
| Profit factor (avg) | 0.4524 |
| Sharpe (avg) | -1.7668 |
| Aggregate score | 44.8959 |
| Runs (set×mode) | 8 |

**Delta win rate:** +27.8252 pp vs baseline.
...
**Family id:** `mt__LinearRegressionRSI_EA`  
**MC rank (by bootstrap median terminal):** **36** / 139  
...
| Historical terminal (compound order of book) | 0.980664× |
| Historical max DD | 2.0091% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.980372× |
| Mean terminal wealth | 0.980119× |
| p05 terminal | 0.961790× |
| p25 terminal | 0.972632× |
| p75 terminal | 0.988112× |
| p95 terminal | 0.997650× |
| P(loss) = P(terminal < 1) | 97.50% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 2.3833% |
| p95 path max DD | 4.0160% |
