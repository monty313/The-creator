# Tweak record: `mt__fasg_trendday_ea`

**Title:** fasg_trendday_ea  
**Kind:** mt  
**Source language:** `strategies/language/01_METATRADER_INDEX.md :: fasg_trendday_ea (.mq5)`  
**Adapter profile:** `fasg`  
...
Profile `fasg` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 70.4794 |
| Total trades (sum set×mode) | 1403 |
| Total return % (avg set×mode) | -0.9186 |
| Max drawdown % (avg) | 1.3544 |
| Profit factor (avg) | 0.8442 |
| Sharpe (avg) | -3.2838 |
| Aggregate score | 83.1664 |
| Runs (set×mode) | 8 |

**Delta win rate:** +29.9933 pp vs baseline.
...
**MC rank (by bootstrap median terminal):** **130** / 139  
...
| Historical terminal (compound order of book) | 0.928544× |
| Historical max DD | 7.2840% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.929340× |
| Mean terminal wealth | 0.929148× |
| p05 terminal | 0.891275× |
| p25 terminal | 0.913559× |
| p75 terminal | 0.943532× |
| p95 terminal | 0.966302× |
| P(loss) = P(terminal < 1) | 99.60% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 7.7912% |
| p95 path max DD | 11.2418% |
