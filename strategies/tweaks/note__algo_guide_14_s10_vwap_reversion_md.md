# Tweak record: `note__algo_guide_14_s10_vwap_reversion_md`

**Title:** s10_vwap_reversion.md  
**Kind:** note  
**Source language:** `C:\Users\user\OneDrive\Desktop\The Creator\strategies\algo_guide_14\s10_vwap_reversion.md`  
**Adapter profile:** `guide_s10_vwap_mr`  
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 40.8648 |
| Note | From prior full-batch / sauces report if present; else 0 |

## What was changed (full detail)

### Tier selected: `E_ultra_breath`

| Parameter | Value | Role |
|-----------|------:|------|
| tp_stop | 0.0001 | Take first unit of progress (~pips on EURUSD) |
| sl_stop | 0.0015 | Invalidate failed thesis |
| max_hold (LTF bars) | 0 | Kill dead trades |
| session 07–21 UTC | False | London/NY concentration |
| HTF strength vs SMA50 | False | Real force, not flat mass |
| bar confirm (close vs open) | True | Candle agrees with side |
| micro structure HL/LH | False | Pullback-resume texture |
...
Guide strategy from Strategies-to-replicate HTML. Profile `guide_s10_vwap_mr` under lab shell.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 91.0417 |
| Total trades (sum set×mode) | 480 |
| Total return % (avg set×mode) | 0.3832 |
| Max drawdown % (avg) | 0.3582 |
| Profit factor (avg) | 25.4242 |
| Sharpe (avg) | 1.7009 |
| Aggregate score | 2542.7144 |
| Runs (set×mode) | 8 |

**Delta win rate:** +50.1768 pp vs baseline.
...
**MC rank (by bootstrap median terminal):** **30** / 139  
...
| Historical terminal (compound order of book) | 0.994871× |
| Historical max DD | 0.5415% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.994952× |
| Mean terminal wealth | 0.994795× |
| p05 terminal | 0.989763× |
| p25 terminal | 0.992672× |
| p75 terminal | 0.996848× |
| p95 terminal | 0.999723× |
| P(loss) = P(terminal < 1) | 95.80% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 0.5492% |
| p95 path max DD | 0.9875% |
