# Tweak record: `mt__CoolBollingerTrendEA`

**Title:** CoolBollingerTrendEA  
**Kind:** mt  
**Source language:** `strategies/language/01_METATRADER_INDEX.md :: CoolBollingerTrendEA (.mq5)`  
**Adapter profile:** `cool_bb`  
...
Profile `cool_bb` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 69.8878 |
| Total trades (sum set×mode) | 902 |
| Total return % (avg set×mode) | -0.8134 |
| Max drawdown % (avg) | 1.0442 |
| Profit factor (avg) | 0.7951 |
| Sharpe (avg) | -3.5803 |
| Aggregate score | 78.4375 |
| Runs (set×mode) | 8 |

**Delta win rate:** +26.8579 pp vs baseline.
...
**Family id:** `mt__CoolBollingerTrendEA`  
**MC rank (by bootstrap median terminal):** **129** / 139  
...
| Historical terminal (compound order of book) | 0.936377× |
| Historical max DD | 6.5566% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.935756× |
| Mean terminal wealth | 0.935821× |
| p05 terminal | 0.905792× |
| p25 terminal | 0.922354× |
| p75 terminal | 0.948130× |
| p95 terminal | 0.967500× |
| P(loss) = P(terminal < 1) | 100.00% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 6.8453% |
| p95 path max DD | 9.8036% |
