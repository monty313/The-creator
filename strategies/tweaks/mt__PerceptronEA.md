# Tweak record: `mt__PerceptronEA`

**Title:** PerceptronEA  
**Kind:** mt  
**Source language:** `strategies/language/01_METATRADER_INDEX.md :: PerceptronEA (.mq5)`  
**Adapter profile:** `macd`  
...
Profile `macd` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 72.2450 |
| Total trades (sum set×mode) | 1018 |
| Total return % (avg set×mode) | -0.3952 |
| Max drawdown % (avg) | 1.0373 |
| Profit factor (avg) | 0.9335 |
| Sharpe (avg) | -1.3860 |
| Aggregate score | 92.7005 |
| Runs (set×mode) | 8 |

**Delta win rate:** +28.1130 pp vs baseline.
...
**Family id:** `mt__PerceptronEA`  
**MC rank (by bootstrap median terminal):** **49** / 139  
...
| Historical terminal (compound order of book) | 0.968437× |
| Historical max DD | 4.0540% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.969085× |
| Mean terminal wealth | 0.968852× |
| p05 terminal | 0.937252× |
| p25 terminal | 0.954059× |
| p75 terminal | 0.982060× |
| p95 terminal | 1.002519× |
| P(loss) = P(terminal < 1) | 93.10% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 4.0714% |
| p95 path max DD | 6.9124% |
