# Tweak record: `note__the_truth_main_extra_strategy_S4_rsi_tension_snap_md`

**Title:** strategy_S4_rsi_tension_snap.md  
**Kind:** note  
**Source language:** `C:\Users\user\OneDrive\Desktop\The Creator\strategies\the_truth_main_extra\strategy_S4_rsi_tension_snap.md`  
**Adapter profile:** `truth_s4_rsi_snap`  
...
Profile `truth_s4_rsi_snap` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 70.0609 |
| Total trades (sum set×mode) | 416 |
| Total return % (avg set×mode) | -0.3821 |
| Max drawdown % (avg) | 0.6024 |
| Profit factor (avg) | 0.8387 |
| Sharpe (avg) | -2.0021 |
| Aggregate score | 83.3342 |
| Runs (set×mode) | 8 |

**Delta win rate:** +30.2978 pp vs baseline.
...
**MC rank (by bootstrap median terminal):** **46** / 139  
...
| Historical terminal (compound order of book) | 0.969488× |
| Historical max DD | 3.7521% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.969408× |
| Mean terminal wealth | 0.969424× |
| p05 terminal | 0.949040× |
| p25 terminal | 0.961226× |
| p75 terminal | 0.977743× |
| p95 terminal | 0.989429× |
| P(loss) = P(terminal < 1) | 99.60% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 3.4101% |
| p95 path max DD | 5.3133% |
