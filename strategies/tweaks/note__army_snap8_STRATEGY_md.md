# Tweak record: `note__army_snap8_STRATEGY_md`

**Title:** STRATEGY.md  
**Kind:** note  
**Source language:** `C:\Users\user\OneDrive\Desktop\The Creator\strategies\army_snap8\STRATEGY.md`  
**Adapter profile:** `snap8`  
...
Profile `snap8` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 71.3992 |
| Total trades (sum set×mode) | 437 |
| Total return % (avg set×mode) | -0.3440 |
| Max drawdown % (avg) | 0.6872 |
| Profit factor (avg) | 12.9255 |
| Sharpe (avg) | -1.9071 |
| Aggregate score | 1292.0345 |
| Runs (set×mode) | 8 |

**Delta win rate:** +30.1359 pp vs baseline.
...
**MC rank (by bootstrap median terminal):** **41** / 139  
...
| Historical terminal (compound order of book) | 0.972467× |
| Historical max DD | 3.2665% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.971810× |
| Mean terminal wealth | 0.972357× |
| p05 terminal | 0.949797× |
| p25 terminal | 0.963449× |
| p75 terminal | 0.981416× |
| p95 terminal | 0.995852× |
| P(loss) = P(terminal < 1) | 97.70% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 3.3074% |
| p95 path max DD | 5.3053% |
