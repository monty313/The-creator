# Tweak record: `mt__Slope_Screener_Fixed`

**Title:** Slope_Screener_Fixed  
**Kind:** mt  
**Source language:** `strategies/language/01_METATRADER_INDEX.md :: Slope_Screener_Fixed (.mq5)`  
**Adapter profile:** `ma_sample`  
...
Profile `ma_sample` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 71.5070 |
| Total trades (sum set×mode) | 1153 |
| Total return % (avg set×mode) | -0.6512 |
| Max drawdown % (avg) | 1.3318 |
| Profit factor (avg) | 0.7472 |
| Sharpe (avg) | -3.3836 |
| Aggregate score | 73.7394 |
| Runs (set×mode) | 8 |

**Delta win rate:** +31.1058 pp vs baseline.
...
**Family id:** `mt__Slope_Screener_Fixed`  
**MC rank (by bootstrap median terminal):** **78** / 139  
...
| Historical terminal (compound order of book) | 0.948933× |
| Historical max DD | 5.2312% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.949688× |
| Mean terminal wealth | 0.949837× |
| p05 terminal | 0.914748× |
| p25 terminal | 0.934581× |
| p75 terminal | 0.964482× |
| p95 terminal | 0.986050× |
| P(loss) = P(terminal < 1) | 99.00% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 5.7484% |
| p95 path max DD | 9.0022% |
