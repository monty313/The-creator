# Tweak record: `note__the_truth_main_extra_strategy_S1_cci_slingshot_md`

**Title:** strategy_S1_cci_slingshot.md  
**Kind:** note  
**Source language:** `C:\Users\user\OneDrive\Desktop\The Creator\strategies\the_truth_main_extra\strategy_S1_cci_slingshot.md`  
**Adapter profile:** `truth_s1_cci`  
...
Profile `truth_s1_cci` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 73.1980 |
| Total trades (sum set×mode) | 390 |
| Total return % (avg set×mode) | -0.3040 |
| Max drawdown % (avg) | 0.5560 |
| Profit factor (avg) | 13.2248 |
| Sharpe (avg) | -1.8011 |
| Aggregate score | 1322.0418 |
| Runs (set×mode) | 8 |

**Delta win rate:** +27.9281 pp vs baseline.
...
**MC rank (by bootstrap median terminal):** **39** / 139  
...
| Historical terminal (compound order of book) | 0.975864× |
| Historical max DD | 2.7654% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.975084× |
| Mean terminal wealth | 0.975441× |
| p05 terminal | 0.954957× |
| p25 terminal | 0.966399× |
| p75 terminal | 0.984131× |
| p95 terminal | 0.997230× |
| P(loss) = P(terminal < 1) | 97.00% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 2.9508% |
| p95 path max DD | 4.8725% |
