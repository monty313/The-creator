# Tweak record: `mt__FTMO_Challenge_EA`

**Title:** FTMO_Challenge_EA  
**Kind:** mt  
**Source language:** `strategies/language/01_METATRADER_INDEX.md :: FTMO_Challenge_EA (.mq5)`  
**Adapter profile:** `challenge`  
...
Profile `challenge` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 69.4649 |
| Total trades (sum set×mode) | 1172 |
| Total return % (avg set×mode) | -0.9494 |
| Max drawdown % (avg) | 1.4066 |
| Profit factor (avg) | 0.7201 |
| Sharpe (avg) | -4.3310 |
| Aggregate score | 70.7071 |
| Runs (set×mode) | 8 |

**Delta win rate:** +26.9002 pp vs baseline.
...
**Family id:** `mt__FTMO_Challenge_EA`  
**MC rank (by bootstrap median terminal):** **133** / 139  
...
| Historical terminal (compound order of book) | 0.926002× |
| Historical max DD | 7.9880% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.926795× |
| Mean terminal wealth | 0.927105× |
| p05 terminal | 0.895289× |
| p25 terminal | 0.912037× |
| p75 terminal | 0.941262× |
| p95 terminal | 0.961207× |
| P(loss) = P(terminal < 1) | 99.80% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 7.8779% |
| p95 path max DD | 10.8498% |
