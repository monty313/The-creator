# Tweak record: `mt__JordanMomentumScreener_v8_HUD`

**Title:** JordanMomentumScreener_v8_HUD  
**Kind:** mt  
**Source language:** `strategies/language/01_METATRADER_INDEX.md :: JordanMomentumScreener_v8_HUD (.mq5)`  
**Adapter profile:** `jordan`  
...
Profile `jordan` keeps this family's original signal language; accuracy layer is the shared Mark filter + TP/SL tier `A_first_breath`.
...
| Metric | Value |
|--------|------:|
| Win rate (accuracy) % | 70.8864 |
| Total trades (sum set×mode) | 882 |
| Total return % (avg set×mode) | -0.4596 |
| Max drawdown % (avg) | 0.8518 |
| Profit factor (avg) | 0.9412 |
| Sharpe (avg) | -1.7456 |
| Aggregate score | 93.4468 |
| Runs (set×mode) | 8 |

**Delta win rate:** +23.9314 pp vs baseline.
...
**Family id:** `mt__JordanMomentumScreener_v8_HUD`  
**MC rank (by bootstrap median terminal):** **64** / 139  
...
| Historical terminal (compound order of book) | 0.963339× |
| Historical max DD | 4.1046% |
...
| Metric | Value |
|--------|------:|
| Median terminal wealth | 0.964203× |
| Mean terminal wealth | 0.963760× |
| p05 terminal | 0.933970× |
| p25 terminal | 0.951699× |
| p75 terminal | 0.976169× |
| p95 terminal | 0.992642× |
| P(loss) = P(terminal < 1) | 98.00% |
| P(max DD ≥ 20%) | 0.00% |
| Median path max DD | 4.2627% |
| p95 path max DD | 6.9182% |
