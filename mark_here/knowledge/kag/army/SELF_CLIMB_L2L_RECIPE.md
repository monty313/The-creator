...
- [x] `best_same > 35` with breach 0  → **36** via `fable_kag_l2l` fire_skill (2026-08-07)
- [ ] `best_same > 36` (next) via `maml_fire_skill_meta` (Reptile/MAML + fire_skill polish)
- [x] skill attributed to path family / laws, not a single MWT day id  
- [x] child SHA floor history unchanged (`9BDCEAAE…`)

**KEEP36 method (reuse forever):** multi-day fire_skill pattern BC + KL + award protect + pack KEEP.  
**Learn-to-learn add-on:** `maml_fire_skill_meta.py` — per-day fire tasks, head adapt, Reptile/FOMAML, then same polish.  
...
**Only as careful path-skill *decode* laws. Not as act-head BC. Not as loose launch fill.**

### Probe evidence (2026-08-06, frozen child SHA `9BDCEAAE…`)

| Config | same | mwt | breach | Note |
|--------|------|-----|--------|------|
| pure child | **35** | 15 | 0 | floor |
| pinn_only (`against_htf`) | 35 | 15 | 0 | 2 bars; floor-neutral |
| thrash + gravity (no entropy) | 35 | 15 | 0 | 4 anti_thrash; floor-neutral |
| gravity + **entropy_hold** | **30** | 20 | 0 | 61 entropy HOLDs kill awards |
| strict_launch + entropy | **30** | 20 | 0 | 64 phys_launch + entropy |
| loose cont (`ltf==htf` fill) | **30** | 20 | 0 | over-launch |

**Verdict:** blanket physics decode does **not** clear >35. Child action geometry is already near its 35 local optimum; global HOLD/fire masks destroy AWARD days. Need bar-precise skill (neural gate / class memory), not doctrine-wide entropy/PINN at score time.

| Pillar | Helps >35 now? | L2L safe? | How wired |
|--------|----------------|-----------|-----------|
| PINN HTF tide penalty | Floor-safe; may not raise alone | Decode only | `against_htf_hold` |
| Entropy regime mask | Floor-safe; may not raise alone | Decode only | `entropy_hold` |
| Kinematic tension | Only with **strict** launch rail | Decode only | `tension_req` + launch |
| Loose cont / Pattern→Buy | **No — drops to 30** | Decode but wrong | banned in seed grid |
| Dimensionless ATR retrain | Later | **No now** | SHA / floor risk |
| PINN in `train_mark_clone_bc` CE | **No for this push** | **Violates base never trained** | defer |

Master equation at score time (not Pattern X → Buy):

```
[High Tension + Low Entropy + HTF Mass OK + doctrine launch/aligned] → Launch with tide
else → wait / HOLD
```

Physics Super’s “inject PINN into BC loss” is **ADD later**, not the R1 climb lever. Climb lever = `self_climb_l2l.py` dials + frozen child.